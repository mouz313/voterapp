                ...
                <div class="mb-3">
                    ...
                    <select name="block_code_id" id="blockCodeSelect" class="form-select @error('block_code_id') is-invalid @enderror">
                        ...
                        @foreach ($blockCodes as $bc)
                            <option value="{{ $bc->id }}" {{ (old('block_code_id', $pollingStation->block_code_id) == $bc->id) ? 'selected' : '' }}>{{ $bc->code }}</option>
                        @endforeach
                    </select>
    ...
    @push('scripts')
        <script>
            (function () {
                ...
                if (ucSelect.value) loadBlockCodes(ucSelect.value, '{{ old('block_code_id', $pollingStation->block_code_id) }}');
            })();
        </script>
    @endpush
@endsection
