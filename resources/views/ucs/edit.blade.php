        ...
        <div class="col-md-8">
            <div class="card shadow-sm">
                <div class="card-header bg-white py-3">
                    <h5 class="mb-0 fw-bold">Edit Union Council: {{ $uc->name }}</h5>
                </div>
                <div class="card-body">
                    <form method="POST" action="{{ route('ucs.update', $uc) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="row g-3 mb-3">
                            <div class="col-md-7">
                                ...
                                <select name="tehsil_id" class="form-select @error('tehsil_id') is-invalid @enderror" required>
                                    ...
                                    @foreach ($tehsils as $tehsil)
                                        <option value="{{ $tehsil->id }}" {{ (old('tehsil_id', $uc->tehsil_id) == $tehsil->id) ? 'selected' : '' }}>
                            ...
                            <div class="col-md-5">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="text" name="uc_no" value="{{ old('uc_no', $uc->uc_no) }}" class="form-control @error('uc_no') is-invalid @enderror" required>
                                </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="national_assembly_id" class="form-select @error('national_assembly_id') is-invalid @enderror">
                                    ...
                                    @foreach ($nationalAssemblies as $na)
                                        <option value="{{ $na->id }}" {{ old('national_assembly_id', $uc->national_assembly_id) == $na->id ? 'selected' : '' }}>
                                            {{ $na->code }} - {{ $na->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="provincial_assembly_id" class="form-select @error('provincial_assembly_id') is-invalid @enderror">
                                    ...
                                    @foreach ($provincialAssemblies as $pa)
                                        <option value="{{ $pa->id }}" {{ old('provincial_assembly_id', $uc->provincial_assembly_id) == $pa->id ? 'selected' : '' }}>
                                            {{ $pa->code }} - {{ $pa->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="name" value="{{ old('name', $uc->name) }}" class="form-control @error('name') is-invalid @enderror" required>
                            @error('name') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        </div>

                        <div class="mb-3">
                            <label class="form-label fw-semibold">UC Name (Urdu / اردو نام)</label>
                            <input type="text" name="name_ur" value="{{ old('name_ur', $uc->name_ur) }}" dir="rtl" class="form-control @error('name_ur') is-invalid @enderror">
                        ...
                        <div class="d-flex justify-content-between">
                            ...
                            <button type="submit" class="btn btn-primary px-4"><i class="bi bi-check-lg me-1"></i> Update Union Council</button>
                        </div>
                    </form>
                </div>
            </div>
